...
import {SafeAreaView, ScrollView,
   ...
   StyleSheet, Dimensions, Platform, ImageBackground, PermissionsAndroid,ActivityIndicator } from 'react-native';
...
import { api_url, GOOGLE_KEY, start_attendence } from "../constant/ApiConstant/Constant";
...
class StartAttendence extends Component{
   ...
   constructor(props){
      ...
      clearInterval(this.default_date);
   }
   ...
   startAttendenceUpdate = async() => {
      console.warn(global.user, this.state.name, this.state.time, this.state.address, this.state.latitude, this.state.longitude,
         this.state.mobile, this.state.start_time, this.state.image);
         ...
         //   };
            ...
            form.append("driver_name", this.state.name);
            form.append("mobile_no", this.state.mobile);
            form.append("start_date_time", this.state.time);
            form.append("start_address", this.state.address);
            form.append("start_time_lat", this.state.latitude);
            form.append("strat_time_lng", this.state.longitude);
            form.append("start_image",{uri:photo.uri,name: 'image.jpg', type: 'image/jpeg'});
           //form.append("start_image",photo);
           
         //    console.warn("ss",form);
            await axios.post(api_url+start_attendence,
               {
                  data: form,
               //    driver_id:global.user,
               //    driver_name:this.state.name,
               //    mobile_no:this.state.mobile,
               //    start_date_time: this.state.time,
               //    start_address: this.state.address,
               //    start_time_lat: this.state.latitude,
               //    strat_time_lng: this.state.longitude,
               //   // start_image:{uri:this.state.image,name: 'image.jpg', type: 'image/jpeg'}
               //    start_image: photo
   ...
   camera =()=>{
      launchCamera(options, (response)=>{
          ...
          if(response.didCancel){
            ...
            //   this.setState({data_img:response.data});
            //   console.log("image data", response.data)
            let path = response.assets.map((path)=>{
                return (
                  
                    this.setState({image:path.uri}) ,
                    this.setState({data_img:path.data}),
                    console.warn("abc",path.data)
                )
            });
          }
      })
   ...
   get_user_profile = async() =>{
      console.warn(global.token, global.user)
      ...
      return(
         ...
         <SafeAreaView style={style.safeAreaView}> 
            ...
            <View style={[style.header, { flexDirection: "row", justifyContent: "space-between" }]}>
               <View style={{ flexDirection: "row", left: 5, justifyContent: "center" }}>
                  ...
                  <View style={{ justifyContent: "center" }}>
                      <Text style={style.headerText}>Start Attendance</Text>
                  </View>
              </View>
            </View>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
               ...
               <View style={[style.input,{height:48,justifyContent:"center"}]}>
                  <Text numberOfLines={1} style={[styles.smallText,{paddingLeft:5}]}>
                     {this.state.address}
                  </Text>
               </View>
               ...
               </Text>
               {
                  ...
                  <TouchableOpacity onPress={()=>this.camera()} style={[style.input,styles.cameraButton]}>
               ...
               <View style={styles.buttonView}>
                  ...
                  {this.state.isLoading ?
                     ...
                     <TouchableOpacity style={[styles.button,{backgroundColor:sky_blue}]}
                        onPress={()=>{this.startAttendenceUpdate()}}>
...
export default StartAttendence;
